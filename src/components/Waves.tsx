...
export default function Waves({
  ...
  useEffect(() => {
    ...
    function movePoints(time: number) {
      ...
      const { waveSpeedX: sx, waveSpeedY: sy, waveAmpX: ax, waveAmpY: ay, friction: fr, tension: tn, maxCursorMove: maxMove } =
        configRef.current;

      lines.forEach((pts) => {
        pts.forEach((p) => {
          const move =
            noise.perlin2((p.x + time * sx) * 0.002, (p.y + time * sy) * 0.0015) * 12;
    ...
    function drawLines() {
      ...
      drawCtx.strokeStyle = configRef.current.lineColor;
