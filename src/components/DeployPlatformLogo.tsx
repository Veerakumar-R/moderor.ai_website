export function DeployPlatformLogo({
  id,
  className = "hero-deploy-logo",
}: {
  id: string;
  className?: string;
}) {
  if (id === "azure") {
    // Official Microsoft Azure "A" mark
    return (
      <svg viewBox="0 0 32 32" className={className} aria-hidden>
        <path
          fill="#0089D6"
          d="M19.867 7.282l-4.733 9.533 8.333 9.66L8 28.23l24 .25zm-.934-3.762L8.067 12.613 0 26.223l6.867-.7z"
        />
      </svg>
    );
  }

  if (id === "gcp") {
    // Official Google Cloud multicolor mark
    return (
      <svg viewBox="0 0 64 64" className={className} aria-hidden>
        <path
          fill="#EA4335"
          d="M40.728 20.488l2.05.035 5.57-5.57.27-2.36C44.2 8.657 38.367 6.26 31.993 6.26c-11.54 0-21.28 7.852-24.163 18.488.608-.424 1.908-.106 1.908-.106l11.13-1.83s.572-.947.862-.9A13.88 13.88 0 0 1 32 17.375c3.3.007 6.34 1.173 8.728 3.102z"
        />
        <path
          fill="#4285F4"
          d="M56.17 24.77c-1.293-4.77-3.958-8.982-7.555-12.177l-7.887 7.887c3.16 2.55 5.187 6.452 5.187 10.82v1.392c3.837 0 6.954 3.124 6.954 6.954 0 3.837-3.124 6.954-6.954 6.954H32.007L30.615 48v8.346l1.392 1.385h13.908A18.11 18.11 0 0 0 64 39.647c-.007-6.155-3.1-11.6-7.83-14.876z"
        />
        <path
          fill="#34A853"
          d="M18.085 57.74h13.9V46.6h-13.9a6.89 6.89 0 0 1-2.862-.622l-2.007.615-5.57 5.57-.488 1.88a18 18 0 0 0 10.926 3.689z"
        />
        <path
          fill="#FBBC05"
          d="M18.085 21.57A18.11 18.11 0 0 0 0 39.654c0 5.873 2.813 11.095 7.166 14.403l8.064-8.064a6.96 6.96 0 0 1-4.099-6.339c0-3.837 3.124-6.954 6.954-6.954 2.82 0 5.244 1.7 6.34 4.1l8.064-8.064c-3.307-4.353-8.53-7.166-14.403-7.166z"
        />
      </svg>
    );
  }

  if (id === "wso2") {
    // Authentic WSO2 orange wordmark (brand mark). Tight viewBox + textLength
    // make the wordmark fill the tile width without distortion.
    return (
      <svg viewBox="0 0 44 22" className={className} aria-hidden>
        <text
          x="22"
          y="12"
          textAnchor="middle"
          dominantBaseline="central"
          textLength="40"
          lengthAdjust="spacingAndGlyphs"
          fill="#FF7300"
          fontFamily="ui-sans-serif, system-ui, -apple-system, Segoe UI, sans-serif"
          fontSize="19"
          fontWeight="800"
        >
          wso2
        </text>
      </svg>
    );
  }

  // On-prem — server / datacenter rack
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden>
      <rect
        x="4"
        y="4"
        width="16"
        height="7"
        rx="1.5"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.5"
      />
      <rect
        x="4"
        y="13"
        width="16"
        height="7"
        rx="1.5"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.5"
      />
      <line x1="7" y1="7.5" x2="13" y2="7.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      <line x1="7" y1="16.5" x2="13" y2="16.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      <circle cx="16.75" cy="7.5" r="1" fill="#FF7A00" />
      <circle cx="16.75" cy="16.5" r="1" fill="#FF7A00" />
    </svg>
  );
}
