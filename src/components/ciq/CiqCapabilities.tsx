...
import { ciqCapabilities } from "@/content/complianceIQ";
import { ScrollReveal } from "../ui/ScrollReveal";
import { SectionLabel } from "../ui/SectionLabel";
import { CiqIcon } from "./icons";
import { CiqSpotlightSilk } from "./CiqSpotlightSilk";
import "./ciq.css";

const EASE = [0.22, 1, 0.36, 1] as const;

export function CiqCapabilities() {
  const { cards, feature } = ciqCapabilities;

  return (
    <section
      id="ciq-capabilities"
      className="aw-capabilities-section ciq-capabilities-section relative px-5 pt-20 pb-20 sm:px-[50px] sm:pt-24 sm:pb-24 lg:pt-28 lg:pb-28"
    ...
    >
      ...
      <div className="relative z-[1] aw-inner">
        <ScrollReveal duration={0.85}>
          <SectionLabel>{ciqCapabilities.eyebrow}</SectionLabel>
        </ScrollReveal>
        <ScrollReveal duration={0.85} delay={0.08}>
          <h2 className="aw-h2">
            {ciqCapabilities.titleLead}
            <span className="text-ember">{ciqCapabilities.titleHighlight}</span>
          </h2>
        </ScrollReveal>
          ...
          >
            ...
            <div className="aw-cap-spotlight-body">
              <span className="aw-cap-spotlight-icon">
                <CiqIcon name={feature.icon} size={24} strokeWidth={1.65} />
              </span>
            ...
            {cards.map((card, i) => (
              ...
              >
                <span className="aw-cap-stat-icon" aria-hidden>
                  ...
                  <CiqIcon name={card.icon} size={18} strokeWidth={1.65} className="aw-cap-stat-icon-svg" />
                </span>
