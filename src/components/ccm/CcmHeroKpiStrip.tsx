...
import { ccmHero } from "@/content/continuousComplianceMonitoring";
...
function KpiMetric({
  ...
  const [display, setDisplay] = useState(reduce || text ? numericValue : 0);
  ...
  }, [inView, numericValue, decimals, reduce, text]);
  ...
  return (
    ...
    >
      <p className="aw-hero-kpi-num">
        {text ?? formatted}
        {!text && suffix ? <span>{suffix}</span> : null}
      </p>
...
export function CcmHeroKpiStrip() {
  const { kpiSection, kpis } = ccmHero;

  return (
    <div className="aw-hero-kpi-section" aria-label="Continuous Compliance Monitoring business outcomes">
      ...
      <div className="aw-hero-kpi-intro">
        ...
        {kpiSection.note ? (
          <ScrollReveal duration={0.85} delay={0.16} className="min-w-0">
            <p className="aw-hero-kpi-note">
              {(Array.isArray(kpiSection.note) ? kpiSection.note : [kpiSection.note]).map((line) => (
        ...
        <div className="aw-hero-kpi-strip">
          {kpis.map((kpi, i) => (
            <KpiMetric
              ...
              text={"text" in kpi ? kpi.text : undefined}
              suffix={kpi.suffix}
              label={kpi.label}
              decimals={
                "decimals" in kpi && typeof kpi.decimals === "number" ? kpi.decimals : undefined
              }
              delay={i * 0.06}
            />
          ))}
