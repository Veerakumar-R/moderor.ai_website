...
import { ccmStakeholders } from "@/content/continuousComplianceMonitoring";
...
export function CcmStakeholders() {
  const reduceMotion = useReducedMotion();
  const [gridReady, setGridReady] = useState(false);
  const { label, titleLead, titleHighlight, cards } = ccmStakeholders;
      ...
      <div className="infra-section-inner mx-auto w-full max-w-[1400px]">
        <div className="infra-box relative w-full overflow-hidden rounded-[28px] bg-[#0c0600] lg:rounded-[36px]">
          ...
          <div className="infra-box-content relative z-[1]">
            ...
            <div className="ccm-sh-cards-grid">
              {cards.map((card, i) => (
                <motion.article
                  key={card.tagline}
                ...
                >
                  ...
                  <h3 className="ccm-sh-card-heading">{card.tagline}</h3>
                  <p className="ccm-sh-card-paragraph">{card.title}</p>
                </motion.article>
              ))}
            </div>
          </div>
