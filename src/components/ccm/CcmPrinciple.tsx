...
import { ccmPrinciple } from "@/content/continuousComplianceMonitoring";
import Waves from "@/components/Waves";
import { PilotHeaderDeco } from "@/components/PilotHeaderDeco";
import { ScrollReveal } from "@/components/ui/ScrollReveal";
import { SectionLabel } from "@/components/ui/SectionLabel";
import { CcmGovernanceVisual } from "./CcmGovernanceVisual";
...
function GovernanceCard({
  ...
  item: (typeof ccmPrinciple.items)[number];
  index: number;
  ...
  return (
    <motion.article
      className={`pilot-story-card pilot-story-card--${theme.variant} ccm-governance-card`}
      style={
        {
      ...
      <div className="pilot-story-card-inner">
        ...
        <div className="pilot-story-scene-wrap">
          <div className="pilot-story-scene-content">
            <CcmGovernanceVisual index={index} reduceMotion={reduceMotion} />
          </div>
        </div>
...
export function CcmPrinciple() {
  const reduceMotion = useReducedMotion();
  const rm = !!reduceMotion;
  const { label, title, description, items } = ccmPrinciple;

  return (
    <section
      id="ccm-governance"
      className="pilot-section ccm-governance-section relative border-b border-border px-5 pt-20 pb-20 sm:px-[50px] sm:pt-24 sm:pb-24 lg:pt-28 lg:pb-28"
      aria-labelledby="ccm-governance-heading"
    >
        ...
        <ScrollReveal duration={0.85}>
          <div className="pilot-header-wrap">
            ...
            <header className="pilot-head-center">
              <h2 id="ccm-governance-heading" className="sr-only">
                {title}
              </h2>
              <SectionLabel className="pilot-section-label">{label}</SectionLabel>
              <p className="pilot-headline ccm-governance-title">{title}</p>
              <p className="pilot-desc ccm-governance-desc">{description}</p>
            </header>
          </div>
        </ScrollReveal>

        <div className="ccm-governance-grid">
