...
import { hrCapabilities } from "@/content/hrCompliance";
import { ScrollReveal } from "../ui/ScrollReveal";
import { SectionLabel } from "../ui/SectionLabel";
import { HrIcon } from "./icons";
import { HrSpotlightSilk } from "./HrSpotlightSilk";
import "./hr.css";

const EASE = [0.22, 1, 0.36, 1] as const;

export function HrCapabilities() {
  const { cards, feature } = hrCapabilities;

  return (
    <section
      id="hr-capabilities"
      className="aw-capabilities-section hr-capabilities-section relative px-5 pt-20 pb-20 sm:px-[50px] sm:pt-24 sm:pb-24 lg:pt-28 lg:pb-28"
    ...
    >
      ...
      <div className="relative z-[1] aw-inner">
        <ScrollReveal duration={0.85}>
          <SectionLabel>{hrCapabilities.eyebrow}</SectionLabel>
        </ScrollReveal>
        <ScrollReveal duration={0.85} delay={0.08}>
          <h2 className="aw-h2">
            {hrCapabilities.titleLead}
            <span className="text-ember">{hrCapabilities.titleHighlight}</span>
          </h2>
        </ScrollReveal>
          ...
          >
            ...
            <div className="aw-cap-spotlight-body">
              <span className="aw-cap-spotlight-icon">
                <HrIcon name={feature.icon} size={24} strokeWidth={1.65} />
              </span>
            ...
            {cards.map((card, i) => (
              ...
              >
                <span className="aw-cap-stat-icon" aria-hidden>
                  ...
                  <HrIcon name={card.icon} size={18} strokeWidth={1.65} className="aw-cap-stat-icon-svg" />
                </span>
