...
import { hrPrinciple } from "@/content/hrCompliance";
import Waves from "@/components/Waves";
import { PilotHeaderDeco } from "@/components/PilotHeaderDeco";
import { ScrollReveal } from "@/components/ui/ScrollReveal";
import { SectionLabel } from "@/components/ui/SectionLabel";
import { HrGovernanceVisual } from "./HrGovernanceVisual";
...
function GovernanceCard({
  ...
  item: (typeof hrPrinciple.items)[number];
  index: number;
  ...
  return (
    <motion.article
      className={`pilot-story-card pilot-story-card--${theme.variant} hr-governance-card`}
      style={
        {
      ...
      <div className="pilot-story-card-inner">
        ...
        <div className="pilot-story-scene-wrap">
          <div className="pilot-story-scene-content">
            <HrGovernanceVisual index={index} reduceMotion={reduceMotion} />
          </div>
        </div>
...
export function HrPrinciple() {
  const reduceMotion = useReducedMotion();
  const rm = !!reduceMotion;
  const { label, title, description, items } = hrPrinciple;

  return (
    <section
      id="hr-governance"
      className="pilot-section hr-governance-section relative border-b border-border px-5 pt-20 pb-20 sm:px-[50px] sm:pt-24 sm:pb-24 lg:pt-28 lg:pb-28"
      aria-labelledby="hr-governance-heading"
    >
        ...
        <ScrollReveal duration={0.85}>
          <div className="pilot-header-wrap">
            ...
            <header className="pilot-head-center">
              <h2 id="hr-governance-heading" className="sr-only">
                {title}
              </h2>
              <SectionLabel className="pilot-section-label">{label}</SectionLabel>
              <p className="pilot-headline hr-governance-title">{title}</p>
              <p className="pilot-desc hr-governance-desc">{description}</p>
            </header>
          </div>
        </ScrollReveal>

        <div className="hr-governance-grid">
