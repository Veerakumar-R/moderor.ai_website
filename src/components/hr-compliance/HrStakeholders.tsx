...
import { hrStakeholders } from "@/content/hrCompliance";
...
export function HrStakeholders() {
  const reduceMotion = useReducedMotion();
  const [gridReady, setGridReady] = useState(false);
  const { label, titleLead, titleHighlight, cards } = hrStakeholders;
      ...
      <div className="infra-section-inner mx-auto w-full max-w-[1400px]">
        <div className="infra-box relative w-full overflow-hidden rounded-[28px] bg-[#0c0600] lg:rounded-[36px]">
          ...
          <div className="infra-box-content relative z-[1]">
            ...
            <div className="hr-sh-cards-grid">
              {cards.map((card, i) => (
                <motion.article
                  key={card.tagline}
                ...
                >
                  ...
                  <h3 className="hr-sh-card-heading">{card.tagline}</h3>
                  <p className="hr-sh-card-paragraph">{card.title}</p>
                </motion.article>
              ))}
            </div>
          </div>
