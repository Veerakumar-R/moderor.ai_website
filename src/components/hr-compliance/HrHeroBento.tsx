...
import { Activity, Bot, ClipboardCheck, MoreVertical, TrendingUp } from "lucide-react";
...
const BAR_MONTHS = [
  { label: "Jan", val: 42, total: 186 },
  { label: "Feb", val: 48, total: 212 },
  { label: "Mar", val: 55, total: 248 },
  { label: "Apr", val: 51, total: 231 },
  { label: "May", val: 58, total: 265 },
];

const LINE_MONTHS = [
  { label: "Nov", val: 28, rate: 11.2 },
  { label: "Dec", val: 34, rate: 13.6 },
  { label: "Jan", val: 38, rate: 15.1 },
  { label: "Feb", val: 44, rate: 17.4 },
  { label: "Mar", val: 41, rate: 16.2 },
  { label: "Apr", val: 47, rate: 18.6 },
  { label: "May", val: 52, rate: 20.1 },
];

const LINE_POINTS = LINE_MONTHS.map((m) => m.val);
const LINE_LABELS = LINE_MONTHS.map((m) => m.label);
const COMPLIANCE_RATES = LINE_MONTHS.map((m) => m.rate);
    ...
    >
      <div className="aw-stack-card-top">
        <div className="aw-stack-hero-label">
          ...
          <div>
            <p className="aw-stack-kicker aw-stack-kicker--accent">Compliance AI</p>
            <p className="aw-stack-sub aw-stack-sub--accent">Policy validation</p>
          </div>
        </div>
        <CardMenu />
      </div>
      ...
      <div className="aw-stack-accent-body">
        <div className="aw-stack-ring aw-stack-ring--accent" aria-hidden>
          <svg viewBox="0 0 64 64">
            <defs>
              <linearGradient id="hr-accent-ring-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
    ...
    >
      <div className="aw-stack-card-top">
        <div className="aw-stack-hero-label">
          ...
          <div>
            <p className="aw-stack-kicker">Failed Checks</p>
            <p className="aw-stack-sub">Across workforce</p>
          </div>
        </div>
        <CardMenu />
      </div>
    ...
    >
      <div className="aw-stack-card-top">
        <div className="aw-stack-hero-label">
          ...
          <div>
            <p className="aw-stack-kicker">Compliance Rate</p>
            <p className="aw-stack-sub">Policy adherence</p>
          </div>
        </div>
        <CardMenu />
      </div>
      ...
      <div className="aw-stack-rate-head">
        <p className="aw-stack-metric aw-stack-metric--sm">
          <Count to={COMPLIANCE_RATES[focus]} decimals={1} />
          <span>%</span>
        </p>
        <div className="aw-stack-rate-stats">
          <span className="aw-stack-rate-pill aw-stack-rate-pill--done">847 compliant</span>
          <span className="aw-stack-rate-pill aw-stack-rate-pill--pending">12 exceptions</span>
        </div>
      </div>
      ...
      <div className="aw-stack-line-wrap aw-stack-line-wrap--rate" aria-hidden>
        <svg className="aw-stack-line-chart" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
          <defs>
            <linearGradient id="hr-rate-area" x1="0" y1="0" x2="0" y2="1">
...
function MiniStatusCard({ reduceMotion }: { reduceMotion: boolean }) {
  const rows = [
    { id: "EXC-2847", status: "Open", tone: "blue" },
    { id: "EXC-2841", status: "Resolved", tone: "grey" },
  ];
    ...
    >
      <div className="aw-stack-card-top">
        <div className="aw-stack-hero-label">
          ...
          <p className="aw-stack-kicker">Open Exceptions</p>
        </div>
        <CardMenu />
      </div>
