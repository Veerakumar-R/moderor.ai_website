...
import { omniStakeholders } from "@/content/omniConnect";
...
export function OmniStakeholders() {
  const reduceMotion = useReducedMotion();
  const [gridReady, setGridReady] = useState(false);
  const { label, titleLead, titleHighlight, cards } = omniStakeholders;
      ...
      <div className="infra-section-inner mx-auto w-full max-w-[1400px]">
        <div className="infra-box relative w-full overflow-hidden rounded-[28px] bg-[#0c0600] lg:rounded-[36px]">
          ...
          <div className="infra-box-content relative z-[1]">
            ...
            <div className="omni-sh-cards-grid">
              {cards.map((card, i) => (
                <motion.article
                  key={card.tagline}
                ...
                >
                  ...
                  <h3 className="omni-sh-card-heading">{card.tagline}</h3>
                  <p className="omni-sh-card-paragraph">{card.title}</p>
                </motion.article>
              ))}
            </div>
          </div>
