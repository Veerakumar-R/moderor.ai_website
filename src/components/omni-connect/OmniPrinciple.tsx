...
import { omniPrinciple } from "@/content/omniConnect";
import Waves from "@/components/Waves";
import { PilotHeaderDeco } from "@/components/PilotHeaderDeco";
import { ScrollReveal } from "@/components/ui/ScrollReveal";
import { SectionLabel } from "@/components/ui/SectionLabel";
import { OmniGovernanceVisual } from "./OmniGovernanceVisual";
...
function GovernanceCard({
  ...
  item: (typeof omniPrinciple.items)[number];
  index: number;
  ...
  return (
    <motion.article
      className={`pilot-story-card pilot-story-card--${theme.variant} omni-governance-card`}
      style={
        {
      ...
      <div className="pilot-story-card-inner">
        ...
        <div className="pilot-story-scene-wrap">
          <div className="pilot-story-scene-content">
            <OmniGovernanceVisual index={index} reduceMotion={reduceMotion} />
          </div>
        </div>
...
export function OmniPrinciple() {
  const reduceMotion = useReducedMotion();
  const rm = !!reduceMotion;
  const { label, title, description, items } = omniPrinciple;

  return (
    <section
      id="omni-governance"
      className="pilot-section omni-governance-section relative border-b border-border px-5 pt-20 pb-20 sm:px-[50px] sm:pt-24 sm:pb-24 lg:pt-28 lg:pb-28"
      aria-labelledby="omni-governance-heading"
    >
        ...
        <ScrollReveal duration={0.85}>
          <div className="pilot-header-wrap">
            ...
            <header className="pilot-head-center">
              <h2 id="omni-governance-heading" className="sr-only">
                {title}
              </h2>
              <SectionLabel className="pilot-section-label">{label}</SectionLabel>
              <p className="pilot-headline omni-governance-title">{title}</p>
              <p className="pilot-desc omni-governance-desc">{description}</p>
            </header>
          </div>
        </ScrollReveal>

        <div className="omni-governance-grid">
