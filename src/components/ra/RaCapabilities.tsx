...
import { raCapabilities } from "@/content/riskAssessment";
import { ScrollReveal } from "../ui/ScrollReveal";
import { SectionLabel } from "../ui/SectionLabel";
import { RaIcon } from "./icons";
import { RaSpotlightSilk } from "./RaSpotlightSilk";
import "./ra.css";

const EASE = [0.22, 1, 0.36, 1] as const;

export function RaCapabilities() {
  const { cards, feature } = raCapabilities;

  return (
    <section
      id="ra-capabilities"
      className="aw-capabilities-section ra-capabilities-section relative px-5 pt-20 pb-20 sm:px-[50px] sm:pt-24 sm:pb-24 lg:pt-28 lg:pb-28"
    ...
    >
      ...
      <div className="relative z-[1] aw-inner">
        <ScrollReveal duration={0.85}>
          <SectionLabel>{raCapabilities.eyebrow}</SectionLabel>
        </ScrollReveal>
        <ScrollReveal duration={0.85} delay={0.08}>
          <h2 className="aw-h2">
            {raCapabilities.titleLead}
            <span className="text-ember">{raCapabilities.titleHighlight}</span>
          </h2>
        </ScrollReveal>
          ...
          >
            ...
            <div className="aw-cap-spotlight-body">
              <span className="aw-cap-spotlight-icon">
                <RaIcon name={feature.icon} size={24} strokeWidth={1.65} />
              </span>
            ...
            {cards.map((card, i) => (
              ...
              >
                <span className="aw-cap-stat-icon" aria-hidden>
                  ...
                  <RaIcon name={card.icon} size={18} strokeWidth={1.65} className="aw-cap-stat-icon-svg" />
                </span>
