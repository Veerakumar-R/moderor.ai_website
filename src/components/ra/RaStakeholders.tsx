...
import { raStakeholders } from "@/content/riskAssessment";
...
export function RaStakeholders() {
  const reduceMotion = useReducedMotion();
  const [gridReady, setGridReady] = useState(false);
  const { label, titleLead, titleHighlight, cards } = raStakeholders;
      ...
      <div className="infra-section-inner mx-auto w-full max-w-[1400px]">
        <div className="infra-box relative w-full overflow-hidden rounded-[28px] bg-[#0c0600] lg:rounded-[36px]">
          ...
          <div className="infra-box-content relative z-[1]">
            ...
            <div className="ra-sh-cards-grid">
              {cards.map((card, i) => (
                <motion.article
                  key={card.tagline}
                ...
                >
                  ...
                  <h3 className="ra-sh-card-heading">{card.tagline}</h3>
                  <p className="ra-sh-card-paragraph">{card.title}</p>
                </motion.article>
              ))}
            </div>
          </div>
