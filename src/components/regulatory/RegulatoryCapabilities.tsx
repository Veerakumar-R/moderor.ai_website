...
import { regulatoryCapabilities } from "@/content/regulatoryCompliance";
import { ScrollReveal } from "../ui/ScrollReveal";
import { SectionLabel } from "../ui/SectionLabel";
import { RegulatoryIcon } from "./icons";
import { RegulatorySpotlightSilk } from "./RegulatorySpotlightSilk";
import "./regulatory.css";

const EASE = [0.22, 1, 0.36, 1] as const;

export function RegulatoryCapabilities() {
  const { cards, feature, eyebrow, titleLead, titleHighlight, description } = regulatoryCapabilities;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-[1] aw-inner">
        <div className="rc-cap-header">
          <ScrollReveal duration={0.85} className="min-w-0">
            <SectionLabel>{eyebrow}</SectionLabel>
            <h2 className="aw-h2 rc-cap-title">
              {titleLead}
          ...
          <ScrollReveal duration={0.85} delay={0.08} className="min-w-0">
            <p className="rc-cap-desc">{description}</p>
          </ScrollReveal>
        </div>
              ...
              >
                <span className="aw-cap-stat-icon" aria-hidden>
                  ...
                  <RegulatoryIcon
                    ...
                    size={18}
                    strokeWidth={1.65}
                    className="aw-cap-stat-icon-svg"
                  />
                </span>
